...
check_gitlab_specific() {
    ...
    if curl -f -s --max-time 10 "http://localhost:9191/users/sign_in" > /dev/null 2>&1; then
        log_success "GitLab web interface: Accessible"
        
        # Check if we can reach GitLab API
        if curl -f -s --max-time 10 "http://localhost:9191/api/v4/version" > /dev/null 2>&1; then
...
check_monitoring_stack() {
    ...
    # Check Grafana
    if curl -f -s "http://localhost:19192/api/health" > /dev/null 2>&1; then
...
check_network_connectivity() {
    ...
    local ports=("5000" "19192" "9090" "9200" "5601" "9191" "9000" "19193")
...
show_useful_urls() {
    ...
    echo "   GitLab API:   http://localhost:9191/api/v4"
